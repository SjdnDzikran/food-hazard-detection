...
\begin{abstract}
Kami berpartisipasi dalam SemEval 2025 Task 9 (Food Hazard Detection), Sub-Task 1, yang menuntut prediksi label hazard-category dan product-category dari judul recall makanan yang sangat pendek. Kami mengusulkan arsitektur dual-path yang menggabungkan encoder Transformer pralatih dengan engineered features ringan (panjang judul, rasio digit, dan lexical cues) pada classification head bersama. Macro F1 adalah metrik utama, dengan kinerja hazard mendominasi skor akhir. Pada dataset latih berisi 5{,}082 judul, model terbaik kami mencapai hazard macro F1 0{,}718, product macro F1 0{,}640 (combined 0{,}679) di validasi, dan combined macro F1 0{,}629 pada test blind. Kami menguraikan preprocessing, setup pelatihan, serta rencana ablation menuju submission akhir di Codalab.
\end{abstract}
...
\section{Pendahuluan}
Deteksi cepat isu keamanan pangan penting untuk mereduksi risiko kesehatan publik. SemEval 2025 Task 9 menargetkan kategorisasi otomatis hazard dan produk dari judul recall yang sangat singkat, dengan distribusi label long-tail dan evaluasi macro F1 yang menekankan akurasi hazard. Pekerjaan ini menjelaskan sistem kami untuk Sub-Task 1 (prediksi kategori) beserta hasil awalnya.

Kontribusi kami: (1) model dual-path yang menggabungkan Transformer text encoder dengan engineered meta-features; (2) penanganan class imbalance memakai loss re-weighting dan augmentasi data untuk hazard minoritas; dan (3) rencana ablation serta analisis error yang selaras dengan metrik kompetisi.
...
Kami memakai train set (5{,}082 sampel) dan validation set (565 sampel); test terdiri dari 997 sampel. Judul sangat singkat (rata-rata $\sim$88 karakter) dan distribusi label sangat imbalanced, terutama pada hazard minoritas (mis. kontaminan kimia tertentu).

\subsection{Gambaran Model}
Kami mengadopsi arsitektur dual-path. Path A adalah Transformer encoder (awal: \texttt{bert-base-uncased}/\texttt{roberta-base}) yang menghasilkan pooled text embedding. Path B memproses engineered features: panjang judul ternormalisasi, rasio digit, keberadaan lexical cues hazard/product, dan country one-hot. Kedua vektor digabung (concatenate) lalu masuk ke MLP bersama yang bercabang menjadi dua classification heads untuk hazard-category dan product-category.

\subsection{Pra-pemrosesan}
Kami lowercase (untuk BERT) atau tetap cased (untuk RoBERTa), menghapus URL dan whitespace ganda, serta truncate/pad hingga 128 token. Country code dipertahankan sebagai fitur kategorikal. Lexicon sederhana (mis. \{salmonella, listeria, aflatoxin\}) dipakai sebagai indikator biner cues.

\subsection{Pengaturan Pelatihan}
Kami fine-tune dengan AdamW, learning rate $2\\text{e-}5$--$5\\text{e-}5$, batch size 16--32, selama 3--5 epoch. Loss menggabungkan dua cross-entropy dengan class weights dari frekuensi label latih; kami juga menguji focal loss. Dropout (0.1--0.2) dipakai pada MLP head. Early stopping memonitor macro F1 di validation set.

\subsection{Baseline dan Variasi}
\begin{itemize}
    \item \textbf{Baseline}: Transformer-only dual-head classifier (tanpa engineered features).
    \item \textbf{Usulan}: Dual-path fusion dengan engineered features.
    \item \textbf{Ablation}: (i) hapus lexical cues; (ii) hapus fitur country; (iii) bandingkan class weights vs focal loss.
\end{itemize}
...
\section{Hasil}
Table~\ref{tab:results} merangkum run terbaik saat ini (dual-path dengan class weights). Macro F1 validasi mencapai puncak 0{,}679 combined, didorong hazard F1 0{,}718. Pada test blind, combined macro F1 0{,}629, dengan product F1 sedikit lebih tinggi dari hazard, mencerminkan tantangan long-tail pada hazard.
...
Model meningkat stabil antar-epoch, dengan hazard F1 memimpin product F1 hingga konvergen. Gap validasi-test pada hazard (0{,}718 ke 0{,}604) mengindikasikan overfitting pada hazard langka dan domain shift antar-split. Product F1 sedikit lebih stabil, kemungkinan karena setiap kategori produk memiliki contoh lebih banyak. Skor combined didorong akurasi hazard, sehingga peningkatan berikutnya harus fokus pada recall hazard minoritas.
...
Kami memaparkan sistem Transformer dual-path untuk SemEval 2025 Task 9 Sub-Task 1. Langkah berikutnya: menyelesaikan pelatihan, memperbarui Table~\ref{tab:results}, menjalankan ablation, dan mengirim prediksi ke Codalab. Pekerjaan lanjutan mencakup pemanfaatan data tak berlabel untuk domain-adaptive pretraining serta memanfaatkan supervisi hazard/product vector dari Sub-Task 2 sebagai auxiliary training.
